<template>
  <div class="py-4">
    ...
    <Dialog ref="productDetailsDialog">
      <div>
        <div class="p-4 rounded-md">
          <img
            v-if="selectedCategory === 'fresas-crema'"
            class="h-auto w-3/4 mx-auto"
            src="../../assets/images/fresas-vaso.png"
            alt="fresas-con-crema"
          />
          <img
            v-else
            class="h-auto w-3/4 mx-auto"
            src="../../assets/images/obleas.png"
            alt="obleas"
          />
        </div>
        <div>
        ...
        <div>
          <div
            v-for="topin in toppings"
            :key="topin.name"
            class="mt-4 last:mb-10"
          >
              ...
              <div v-for="option in topin.options" :key="option">
                <div class="flex flex-row justify-between">
                  ...
                  <input
                    class="w-[25px] h-10"
                    type="checkbox"
                    v-model="toppingsSelected[topin.name]"
                    :id="option"
                    :value="option"
                  />
                </div>
            ...
            <template v-else-if="topin.type === 'qty'">
              <div
                v-for="(option, optIdx) in topin.options"
                :key="option"
                class="mt-2"
              >
                <div class="flex flex-row justify-between">
                  ...
                  <div
                    class="flex flex-row items-center gap-2 my-1 text-2xl rounded-full border px-4"
                  >
                    <template
                      v-if="
                        toppingsSelected[topin.name]['options'][optIdx].qty > 0
                      "
                    >
                      <button
                        @click="
                          toppingsSelected[topin.name]['options'][optIdx].qty--
                        "
                        class="font-bold"
                      >
                        -
                    ...
                    <button
                      @click="handleMoreToppingQuantity(topin.name, optIdx)"
                      class="font-bold"
                    >
                      +
                    </button>
        ...
        <div>
          <button
            ...
            @click="productDetailsDialog.closeModal()"
          >
            Cancelar
          </button>
        </div>
    ...
    <div v-if="products.length">
      <div
        class="border rounded-xl my-4"
        v-for="category in products" :key="category.categoryId">
        <h2 class="text-xl font-bold mt-4 px-6 text-black">{{ category.category }}</h2>
        <ProductListItem
          v-for="product in category.products"
          :category="category.categoryId"
          :key="product.name"
          :product="product"
          @productSelected="handleProductSelected($event, category.categoryId)"
        />
      </div>
    </div>
    <div v-else>
...
function handleProductSelected(product, category) {
